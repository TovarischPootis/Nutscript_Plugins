...
function PANEL:Init()
    if nut.gui.cinematicSplashText then
        nut.gui.cinematicSplashText:Remove()
    end

    nut.gui.cinematicSplashText = self
...
function PANEL:TriggerText()
    ...
    if contents.music then
        music = CreateSound(LocalPlayer(), nut.config.get("cinematicTextMusic","music/stingers/industrial_suspense2.wav"))
        music:PlayEx(0, 100)
