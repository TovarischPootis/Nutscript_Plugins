...
function PANEL:Init()
    ...
    if IsValid(nut.gui.cinematicSplashTextMenu) then
        nut.gui.cinematicSplashTextMenu:Remove()
    end
    nut.gui.cinematicSplashTextMenu = self
    ...
    durationLabel:SetTextColor(nut.config.get("color", Color(75, 119, 190)))
    ...
    postButton.DoClick = function()
        if not (self.contents and (self.contents.text or self.contents.bigText)) then nut.util.notify("Something went horribly wrong. Try reloading this panel") return end
        if  self.contents.text == "" and self.contents.bigText == "" then nut.util.notify("Text is missing. Enter some text to display") return end
        ...
        net.SendToServer()
        nut.util.notify("Splash Text Sent")
        self:Remove()
    end
    self:SizeToContents()
