...
nut.config.add("Crosshair", true, "Whether the Crosshair is enabled or not.", nil, {
	category = "Crosshair"
})
...
local aimVector, punchAngle, ft, screen, distance
...
function PLUGIN:PostDrawHUD()
	if not nut.config.get("Crosshair", true) then
		return
	end

	local client = LocalPlayer()
	if (not client:getChar() or not client:Alive()) then
		return
	end
...
function PLUGIN:GetCrosshairIcon(curAlpha, entity, weapon, distance)
	...
	if (IsValid(wep)) and (wep:GetNW2Bool("holdingObject", false) == true) then
		return ""
	end
	...
	if (IsValid(entity) and distance < 16384) and (not entity:IsPlayer() and not entity:IsNPC()) then
		...
		if (not nut.xhair.entIgnore[class]) then
